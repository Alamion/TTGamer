...
- [ ] ⬜ **T-012 — Database + auth** (T-067) — players and GMs optionally sync and share sheets across devices with authenticated accounts while keeping an offline-first local cache and explicit conflict/recovery behavior. (task for roadmap path `multiplayer-groups`, `online-forum`)
- [ ] ⬜ **T-037 — Third-party text audit & licensing notices** (none) — readers and rights holders get documentation and catalog text written in the project's own words, with each system's required notices shown where its material is used. (task for roadmap path `core-book-docs`)
    ...
    - Notices become system/ruleset metadata rendered automatically (Dark Pack for V5 content, per-publisher policies for others); no notice on unrelated pages.
    ...
    - 2026-09-25: decided — no. The Dark Pack grants rights to World of Darkness 5th Edition material only, so neither the Star Wars WoD 2e conversion nor the classic WoD 2e engine (spec 012) declares it; constitution 1.4.2 records the scope.
- [x] ✅ **T-038 — V5 ruleset + Hunter: the Reckoning 5e player character** (none) — players create, edit, and export a validated H:tR 5e hunter sheet built on a reusable V5 ruleset layer, and can re-skin it for a homebrew setting through the template system. (task for roadmap path `multi-system-sheets`)
    - Highest priority: a table game runs on it within 1–2 weeks of 2026-09-15; keep scope to what that session needs.
    - Source: `context/corebooks/wod/Hunter the reckoning 5e.pdf` (image-only scan; read pages visually). Sheet structure and trait names only — no verbatim rules text.
    ...
    - 2026-09-15: implemented by spec 008 (V5 ruleset + Hunter module, full/brief sheets on the existing WoD elements, Dark Pack badge, V5/Hunter docs en + ru); in-app review of the quickstart scenarios passed on 2026-09-19 with all found bugs fixed.
...
- [ ] ⬜ **T-067 — Truly offline installable app concept** (T-052) — players and GMs install the tool as a standalone app that runs with no network and no hosted site — for example a PWA, or a similar wrapper that can be published as a Google Play listing — instead of depending on the website being reachable. (task for roadmap path `app-packaging`)
    - A large task: starts as a concept (PWA vs. native wrapper such as a TWA/Capacitor/Tauri build, update and storage-durability behavior, store requirements) before any implementation spec.
    - Queued after the file system (T-052) and before multiplayer (T-012), so storage and packaging settle before sync is designed on top of them.
- [ ] ⬜ **T-053 — Guided step-by-step character creation** (none) — players build a legal character through ordered steps with the rules applied as they go: the current step highlighted, allowed ranges shown, and per-category budgets (for example skill dots) enforced instead of a blank sheet. (task for roadmap path `character-creation-flow`)
    - Also owns the in-app system quickstart: the same stepper walks a newcomer through one system's basics, so the docs quickstart and the creation flow do not drift apart.
    - Acceptance includes the hallway test inherited from T-038 (SC-009): someone who has never played the system creates a legal character unaided, with no explanation from the maintainer.
...
- [x] ✅ **T-068 — Template editor guide with in-editor help** (none) — template authors understand every editor setting: a template editor section under `docs/` explains the elements and settings, and the editor links to the matching fragment through the existing documentation-link mechanism. (task for roadmap path `gm-notes-templates`)
    ...
    - 2026-09-25: done. Guide `docs/template-editor/` (overview, elements, values and conditions, types and settings; Russian mirror) with fixed heading anchors; a "?" next to the guide in the editor header and beside the documentation link, shared value keys, value source, formulas, limits, display conditions, catalogs, columns, and the type-and-setting select. Documentation links accept `/docs/…` (opened in the reader's language — shipped links used to open the English page for Russian readers) or `https://`; other values (including `javascript:`) are editor issues and never render.
- [x] ✅ **T-069 — Element storybook** (none) — contributors review every template element variant, documentation widget, and app color on one draft-only docs structure (`draft: true`, hidden in production like `docs/TODO.md`), so a change to an element is checked in one place. (task for roadmap path `gm-notes-templates`)
    ...
    - Include the `DocsHelpLink` atom (site, external, and rejected links).
    - Constitution VI (v1.5.0) makes the storybook mandatory for new and changed elements.
    - 2026-09-25: done. `docs/dev/storybook/` (draft): template elements and built-in parts render `src/sheet_manager/storybook/stories.ts` on sandbox documents (built-in parts generated per system from binding shapes), documentation widgets, and `Palette` (Tailwind theme colors from the config plus every color variable, both themes). `tests/sheet_manager/storybook.test.tsx` requires every field type, option, container, and binding shape. Not covered automatically: raw Tailwind default colors used in class names (for example `bg-black/50`, `dark:bg-gray-800`).
- [x] ✅ **T-070 — Retarget a template's type and setting** (none) — template authors move an existing template to another document type or setting instead of recreating it; values bound to coordinates the new target lacks are reported, not silently dropped. Matters most for user types and settings.
    - 2026-09-25: done. The editor header's "Type and setting" select lists shipped settings (with the user types they own) and user settings (core definitions and own types); on save, documents assigned to the page that it can no longer render return to their default page after a confirmation, user setting pages follow the page, and a type whose default page left picks another. Bindings the new target lacks are draft issues that block the save. The library's "New template" select uses the same grouped list.
- [x] ✅ **T-071 — Separate type and setting manager** (T-070) — users manage their settings and document types (create for each other, rename, update, delete) in a tree like the editor outline, apart from template management, and see at a glance which setting owns each type. (task for roadmap path `gm-notes-templates`)
...
- [ ] ⬜ **T-073 — Rating element parity with trait rows** (T-058) — a template rating looks and works like the attribute and skill rows. (task for roadmap path `gm-notes-templates`)
    ...
    - A maximum from a value or formula (`maxFrom`) above the static maximum shows the extra dots but refuses writes above the static maximum (storybook: 30 dots, only 10 selectable). The computed maximum should decide, up to the schema limit.
    - Keep the compressed "grain" look of many dots (dozens of dots narrow into pills in one row), and give every dot a hitbox that fills its whole cell up to the neighbours, with no gap between hitboxes, while the round shape and spacing stay as they are.
...
- [x] ✅ **T-081 — Primary and secondary accents in the template editor** (none) — the template editor and the library use the app's primary and secondary colors for selection, frames, pressed modes, and help, like the rest of the app; the violet becomes a `tertiary` token kept for edge cases (for example parts added automatically), per the accent roles in the theming skill. (task for roadmap path `gm-notes-templates`)
    - Today about 20 editor classes use `editor` (violet) as the main accent: the Edit/Preview switch, element frames and chips, insert slots, the outline selection.
    - Include the storybook palette (the renamed token) and the draft-only storybook pages.
- [ ] ⬜ **T-082 — Core characters in shipped line settings** (T-071) — users give a ruleset's core character (the V5 mortal) its own pages inside a shipped line setting such as Hunter, the way they already can inside their own settings, and see it listed there in the library. (task for roadmap path `gm-notes-templates`)
    - Deferred from spec 013 (research R3): documents record only user settings in `metadata.settingId`, so a mortal "inside Hunter" has no stored meaning yet; the approved library prototype showed that row.
- [ ] ⬜ **T-079 — System-aware roll links in docs** (none) — documentation readers roll their open document's stats from any system: `CharRoll` reads values through the document's bindings (trait and resource coordinates, template value keys of user types and settings) and builds the pool with the system's dice rule, instead of the Star Wars character shape and a fixed `d10>=6 f=1`. (task for roadmap path `core-book-docs`)
    - Today it only understands Star Wars characters; on a V5 or WoD 2e document it rolls nothing or the wrong pool.
- [ ] ⬜ **T-080 — One catalog table for documentation** (none) — documentation authors show every catalog through `CatalogBrowser` (a catalog registered by its system, localized from the catalog translations): the Star Wars catalogs declare `browse`, and the hand-configured `DataCatalog` embeds with `src/data/*Config.tsx` in the Star Wars docs are replaced. (task for roadmap path `core-book-docs`)
    - Today both render the same table: `CatalogBrowser` wraps `DataCatalog` and reads a registered catalog, while `DataCatalog` embeds pass raw data, columns, and renderers from MDX (the older Star Wars path). The 11 Star Wars catalogs have no `browse`.
...
- [x] ✅ **T-021 — UI i18n migration** (T-020) — the whole interface becomes translatable: the remaining sheet, dice, shared, site-shell, catalog, and integration strings migrate in small domain batches using generated descriptors and `<Translate>` where suitable; no second locale runtime.
    - 2026-09-19: done in spec 009 (home page, dice roller, Discord messages, shared components, catalog configs, sheet controls, Russian plurals); the verifier's `interface` rule gates new literals.
- [ ] 🟡 **T-022 — Catalog data i18n migration** (T-021) — catalog entries display in the reader's language: user-facing fields of each catalog localize, then DataCatalog detail/search/filter configurations use localized values with English fallback.
    - 2026-09-19: spec 009 localized every Star Wars catalog name, short description, list, and enumerated label, and the DataCatalog configs; long `description` fields remain English (T-063). Remaining: in-app review of the new Russian names (see the glossary notes).
    - 2026-09-23: the T-061 review also settled the catalog names (Force powers are «Техники Силы», species/creature/vehicle spellings follow the established Russian forms). Remaining: the maintainer's own pass through the app, then long descriptions (T-063).
...
- [x] ✅ **T-061 — V5/Hunter Russian terminology review** (none) — Russian readers get consistent, reviewed H:tR 5e terms (module name, Storyteller, Touchstones, Edges/Perks, Creed/Drive, Advantage and gear names) across the sheet UI, catalogs, and `docs/wod-v5`; one glossary decides each term and every source follows it.
    ...
    - Review checklist (spec 009 T100):
        ...
        - 2026-09-23: reviewed and applied. Strength is «Мощь» in Star Wars (Сила names the Force), Willpower «Воля», Conviction «Убеждённость», the V5 Awareness skill «Бдительность», the Star Wars Awareness merit category «Чутьё», Aptitudes «Способности»; Charisma/Brawl/Melee/Subterfuge stay per setting; short forms shortened («Рук. оружие», «Прониц.», «Гум. науки», «Програм.»). Sources, Russian docs and the terminology catalog follow the glossary, and `glossary` and `catalog` are now gated to `error`.

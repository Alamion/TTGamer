...
- [ ] ⬜ **T-066 — Display-condition 3D dice tests** (none) — 3D roll regressions that only show on some machines are caught in CI: the renderer loop runs headless (real cannon-es, stubbed WebGL) under a fake clock simulating 30/60/144/165/240 Hz and irregular frame times (stutter, background-tab throttling), asserting no interpenetration after spawn, bounded body speeds, settle only when bodies are at rest, and the same wall-clock show/fade durations at every rate; the cases reproduce F-004 and F-005 before they are fixed.

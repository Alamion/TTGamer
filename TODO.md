...
- [ ] 🟡 **T-038 — V5 ruleset + Hunter: the Reckoning 5e player character** (none) — players create, edit, and export a validated H:tR 5e hunter sheet built on a reusable V5 ruleset layer, and can re-skin it for a homebrew setting through the template system. (task for roadmap path `multi-system-sheets`)
    ...
    - 2026-09-15: implemented by spec 008 (V5 ruleset + Hunter module, full/brief sheets on the existing WoD elements, Dark Pack badge, V5/Hunter docs en + ru); in-app review of the quickstart scenarios passed on 2026-09-19 with all found bugs fixed; remaining: the newcomer hallway test before marking done.

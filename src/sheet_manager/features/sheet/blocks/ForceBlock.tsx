...
import { starWarsForceSkills, starWarsVirtues } from '../../../systems/star-wars-wod';
...
export function ForceBlock({ accentColor = 'secondary' }: ForceBlockProps) {
    ...
    return (
        <CollapsibleBlock
            title={isMechanical ? translate(uiMessages.sheet.documents.fields.resources) : 'Force'}
            accentColor={accentColor}
            storageKey="forceBlock"
            docsPath={
                isMechanical
                    ? '/docs/star-wars-wod-2e/character/virtues-willpower'
                    : '/docs/star-wars-wod-2e/character/force'
            }
        >
            <div
                className={
            ...
            >
                {!isMechanical && (
                    ...
                    >
                        {starWarsForceSkills.traits.map((skill) => {
                            const trait = character.forceSkills?.[skill.key] || {
                                ...DEFAULT_SKILL_VALUE,
                            };
                            return (
                                <TraitRowWithInput
                                    ...
                                    onChange={(val, spec, exp, prc) =>
                                        handleForceSkillChange(skill.key, val, spec, exp, prc)
                                    }
                                    onSpecializationTextChange={(text) =>
                                        handleForceSkillSpecializationChange(skill.key, text)
                                    }
                                    size="md"
                ...
                <div className="space-y-4">
                    ...
                    >
                        {starWarsVirtues.traits.map((virtue) => {
                            const trait = character.virtues?.[virtue.key] || {
                            ...
                            return (
                                <TraitRowWithInput
                                    ...
                                    value={trait.value}
                                    onChange={(val, spec, exp, prc) =>
                                        handleVirtueChange(virtue.key, val, spec, exp, prc)
                                    }
                                    onSpecializationTextChange={(text) =>
                                        handleVirtueSpecializationChange(virtue.key, text)
                                    }
                                    size="md"
                                    minimal={1}
                                    onDiceRoll={buildDiceNotation}

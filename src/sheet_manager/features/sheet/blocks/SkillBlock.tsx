...
import { starWarsAbilityGroups } from '../../../systems/star-wars-wod';
import type { CustomSkill } from '../../../types/character';
import { DEFAULT_SKILL_VALUE } from '../../../types/character';

type SkillCategory = 'talents' | 'skills' | 'knowledges';
...
export function SkillBlock({ accentColor = 'secondary' }: SkillBlockProps) {
    ...
    const renderSkillColumn = (group: (typeof starWarsAbilityGroups)[number]) => {
                    ...
                    return (
                        <TraitRowWithInput
                            ...
                            onChange={(val, spec, exp, prc) =>
                                handleSkillChange(skill.key, val, spec, exp, prc)
                            }
                            onSpecializationTextChange={(text) =>
                                handleSkillSpecializationChange(skill.key, text)
                            }
                            size="md"
                            showFlags={true}
    ...
    return (
        ...
        >
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {starWarsAbilityGroups.map(renderSkillColumn)}
            </div>
        </CollapsibleBlock>
    );
}

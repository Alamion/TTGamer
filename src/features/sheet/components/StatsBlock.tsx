...
export function DerivedStatsBlock() {
  ...
  const dex = currentCharacter.attributes.Dexterity || 0;
  const wits = currentCharacter.attributes.Wits || 0;
  const str = currentCharacter.attributes.Strength || 0;
  const con = currentCharacter.attributes.Constitution || 0;
  const perce = currentCharacter.attributes.Perception || 0;
  const int = currentCharacter.attributes.Intelligence || 0;

  const initiative = dex + wits;
  const dodge = dex + (currentCharacter.skills.Dodge || 0);
  const soak = con + (currentCharacter.attributes.Stamina || con);
      ...
      <div className="space-y-3">
        ...
        <div className="flex justify-between items-center">
          ...
          <input
            type="number"
            value={spentXP}
            onChange={(e) => setSpentXP(parseInt(e.target.value) || 0)}
            className="w-20 bg-slate-900 border border-slate-700 rounded px-3 py-1.5 text-right text-slate-200 font-mono [-moz-appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
          />
        </div>
        <div className="flex justify-between pt-2 border-t border-slate-700">
          <span className="text-slate-400 text-sm">Available</span>
          <span className="text-slate-200 font-mono">{totalXP - spentXP}</span>
        </div>
      </div>
    </div>
  );

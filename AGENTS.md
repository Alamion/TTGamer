...
- Element storybook (constitution VI): every template element variant, documentation widget, and palette color appears in the draft-only docs storybook (T-069); a new or changed element is not done until it does. A setting's new element is an editor-configurable template element, preferably an option on an existing element rather than a similar new one.

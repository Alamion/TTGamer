          ...
          <tr class="bg-gray-100">
            ${columns.map(
              (col) => html`
                ...
                >
                  ${col.label}
                  ${sortColumn.value === col.key && (sortDirection.value === 'asc' ? ' ↑' : ' ↓')}
                </th>
              `
            )}
          </tr>
          ...
          ${sortedData.value.map(
            (row, i) => html`
              <tr key=${i} class="border-t hover:bg-gray-50">
                ${columns.map(
                  (col) => html` <td key=${col.key} class="px-4 py-2">${row[col.key]}</td> `
                )}
              </tr>
            `
          )}
  ...
  return html`
    <div
      class=${`border-2 border-dashed rounded-lg p-8 text-center transition-colors ${
        isDragging.value ? 'border-blue-500 bg-blue-50' : 'border-gray-300 bg-gray-50'
      }`}
    ...
    >
      ...
      <label for="file-input" class="cursor-pointer">
        <div class="text-gray-600">
          ...
          <p class="text-sm">${accept === '*' ? 'Any file type' : `Accepts: ${accept}`}</p>
        </div>
      </label>
...
function Tabs({ tabs }) {
  ...
  return html`
    <div class="w-full">
      ...
      <div class="p-4">${tabs[activeTab.value]?.content}</div>
    </div>
  `;
}
      ...
      ${toasts.value.map((toast) => {
        ...
        return html`
          ...
          >
            ...
            <button onClick=${() => removeToast(toast.id)} class="text-white hover:text-gray-200">
              ✕
            </button>
          </div>
        `;
...
function CSVUploader({ onDataParsed }) {
  ...
  return html`
    <div class="space-y-4">
      ...
      />
      ${status.value && html` <p class="text-sm text-gray-600">${status}</p> `}
    </div>
  `;
}
